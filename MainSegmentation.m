...
filesHaemorrhages  = dir(fullfile(folderHaemorrhages,   '*.tif')); % QUI LAVORIAMO DENTRO LA SOTTOCARTELLA HAEMORRHAGES
...
if isempty(filesHaemorrhages) || isempty(filesMicroaneurysms) || ...
   isempty(filesHardExudates)  || isempty(filesOpticDisc) || isempty(filesSoftExudates)
    error('Cartella non trovata o vuota');
end

% APPLICO IL PREPROCESSING AD UN NUMERO DI IMMAGINI A NOSTRA SCELTA
%  results = cell(1,5); DEVONO ESSERE UGUALI!!
numImmagini = 5;
...
for k = 1:numImmagini
    ...
    indexHaemorrhages  = find(contains({filesHaemorrhages.name},  nomeBase));
    ...
    if ~isempty(indexMicroaneurysm) && ~isempty(indexHaemorrhages) && ...
       ~isempty(indexHardExudates)  && ~isempty(indexOpticDisc) && ~isempty(indexSoftExudates)
        ...
        gtPathHaemorrhages   = fullfile(filesHaemorrhages(indexHaemorrhages).folder,    filesHaemorrhages(indexHaemorrhages).name);
        ...
        % Recupero le due segmentazioni dalla cella Nx3
        ...
        subplot(2,4,1);
        imshow(startingImg);
        title(['Originale: ', files(k).name], 'FontSize', 8);
        ...
        title(['GT: ', filesHaemorrhages(indexHaemorrhages).name], 'FontSize', 8);
